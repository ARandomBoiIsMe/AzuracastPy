...
from AzuracastPy.util.general_util import get_day_number
from AzuracastPy.exceptions import ClientException
from AzuracastPy.constants import API_ENDPOINTS, WEBHOOK_CONFIG_TEMPLATES
...
class PlaylistHelper:
    ...
    def create(
        ...
        if not isinstance(type, PlaylistTypes):
            message = f"type param must be one of: {', '.join(PlaylistTypes.__members__)}"
            raise ClientException(message)
        
        type = type.value
        
        if not isinstance(source, PlaylistSources):
            message = f"source param must be one of: {', '.join(PlaylistSources.__members__)}"
            raise ClientException(message)
        
        source = source.value
        
        if not isinstance(order, PlaylistOrders):
            message = f"order param must be one of: {', '.join(PlaylistOrders.__members__)}"
            raise ClientException(message)
        
        order = order.value
        
        if not isinstance(remote_type, PlaylistRemoteTypes):
            message = f"remote_type param must be one of: {', '.join(PlaylistRemoteTypes.__members__)}"
            raise ClientException(message)
...
class WebhookHelper:
    ...
    def create(
        ...
        if not isinstance(type, WebhookConfigTypes):
            message = f"type param must be an attribute from the WebhookConfigTypes enum class."
            raise ClientException(message)
